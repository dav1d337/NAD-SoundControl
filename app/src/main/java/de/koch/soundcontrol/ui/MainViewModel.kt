...
class MainViewModel : ViewModel() {
    ...
    fun createRetrofitClient(ip: String, port: String): Boolean {
        return try {
            if (ip.isNotEmpty() && port.isDigitsOnly() && port.toInt() >= 0 && port.toInt() < 65535) {
    ...
    private fun callPost(lazyCall: Call<NADResponse>?, ifSuccess: () -> Unit) {
        if (client == null) {
            error.value = "IP Config not set"
            return
        }
        loading.value = true
        lazyCall?.enqueue(object : Callback<NADResponse> {
    ...
    private fun callGet(lazyCall: Call<NADResponse>?, target: MutableLiveData<*>) {
        if (client == null) {
            error.value = "IP Config not set"
            return
        }
